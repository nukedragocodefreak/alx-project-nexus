...
const FEEDS: FeedConfig[] = [
  ...
  { id: "discover", label: "Discover", icon: SlidersHorizontal },
  { id: "tv_popular", label: "TV", icon: Tv },
  { id: "watchlist", label: "Watchlist", icon: Clock },
];

const TRENDING_MEDIA_TYPES: Array<{ id: "movie" | "tv"; label: string }> = [
  { id: "movie", label: "Movies" },
  { id: "tv", label: "TV" },
];

const TRENDING_WINDOWS: Array<{ id: "day" | "week"; label: string }> = [
  { id: "day", label: "Today" },
...
export default function MovieRecommendationsUI() {
  ...
  useEffect(() => {
    fetchJSON("/api/tmdb?fn=genre_list")
      .then((data: { genres?: Array<{ id: number; name: string }> }) => {
  ...
  useEffect(() => {
    fetchJSON("/api/tmdb?fn=configuration")
      .then((data: TmdbConfiguration) => {
  ...
  useEffect(() => {
    ...
    const endpoint =
      selected.mediaType === "tv"
        ? `/api/tmdb?fn=tv_details&tvId=${selected.id}`
        : `/api/tmdb?fn=movie_details&movieId=${selected.id}`;
  ...
  const currentList = useMemo(() => {
    ...
    return movies;
  }, [activeFeed, watchlist, catalog, movies]);

  const filtered = useMemo(() => {
    return currentList.filter((movie) => {
      const matchesRating = movie.rating >= minRating;
      const matchesGenres =
        activeGenres.length === 0 || activeGenres.every((g) => movie.genres.includes(g));
      return matchesRating && matchesGenres;
    });
  }, [currentList, minRating, activeGenres]);

  const selectedMovie = useMemo(() => {
    if (!selected) return null;
    return catalog[selected.id] || null;
  }, [selected, catalog]);
  ...
  const preferredPosterSize = configuration?.images?.poster_sizes?.find((size) => size === "w342")
  ...
  return (
    <div>
      <HeaderWrap>
        <Container>
          <Brand>
            ...
            <div>
              <div style={{ fontWeight: 700 }}>SceneScout</div>
      ...
      <Main>
        <div style={{ position: "sticky", top: 100, display: "grid", gap: 16, height: "fit-content" }}>
          <Card>
            <CardHeader>
              <CardTitle>Refine</CardTitle>
              <CardDescription>Tune the recommendation feed.</CardDescription>
            </CardHeader>
              ...
              <div>
                ...
                <GenrePills>
                  {genreChoices.map((genre) => (
                    <Pill key={genre} active={activeGenres.includes(genre)} onClick={() => toggleGenre(genre)}>
                      {genre}
                      {activeGenres.includes(genre) && <X size={14} />}
                    </Pill>
                  ))}
                </GenrePills>
              </div>
              ...
              {configuration ? (
                <>
                  <div>
                    ...
                    <div>{configuration.images?.secure_base_url || "Unavailable"}</div>
                  </div>
                  <div>
                ...
                <Muted>Configuration unavailable: {configurationError}</Muted>
              ) : (
            ...
            <Card>
              <CardBody style={{ display: "flex", flexWrap: "wrap", gap: 12, alignItems: "center" }}>
                ...
                <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                  <label style={{ fontSize: 12, color: theme.colors.subtext }}>Type</label>
                  <select
                    value={trendingMediaType}
                    onChange={(event) => setTrendingMediaType(event.target.value as "movie" | "tv")}
                    style={{
                  ...
                  >
                    {TRENDING_MEDIA_TYPES.map((option) => (
                ...
                <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                  ...
                  <select
                    ...
                    onChange={(event) => setTrendingWindow(event.target.value as "day" | "week")}
                    style={{
                      border: `1px solid ${theme.colors.border}`,
                      borderRadius: 8,
                      padding: "6px 10px",
